...
import { Package, FileText, FileUp, Paperclip, Copy, Archive, CheckCircle2, Trash2, Eye, ChevronRight } from 'lucide-react'
          ...
          {filtered.map((s) => (
            <div key={s.id} className="version-card">
              ...
              <div className="version-card-body">
                {s.appliedAt && <span className="text-soft">应用时间: {new Date(s.appliedAt).toLocaleString('zh-CN')} · </span>}
                {s.tableIds?.length > 0 && <span className="text-soft">涉及 {s.tableIds.length} 张表 · </span>}
